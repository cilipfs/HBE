...
import sk.suchac.hbe.objects.HistoryRecordComparator;
import android.app.Activity;
import android.content.SharedPreferences;

public class HistoryHelper {
	
	public static final String PREFS_HISTORY = "HbeHistoryPrefs";
	public static final int MAX_RECORDS = 10;
	
	public static boolean saveRecord(Activity activity, int bookIndex, int chapterIndex) {
		boolean success = false;
		SharedPreferences settings = activity.getSharedPreferences(PREFS_HISTORY, 0);
		Map<String,?> internal = settings.getAll();
		
		long timestamp = new Date().getTime();
		HistoryRecord histRecord = new HistoryRecord(timestamp, bookIndex, chapterIndex);
